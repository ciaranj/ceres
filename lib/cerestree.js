...
var walk = function(dir, done) {
  ...
  fs.readdir(dir, function(err, list) {
    ...
    list= list.filter( function(file) {
      return !( file.indexOf(".slice") > 0 || file.indexOf(".ceres-node") > 0 );
    });
...
CeresTree.prototype.getNodePath = function(fsPath) {
  ...
  var nodePath= fsPath.substring( this.root.length+1 ).split(path.sep).join(".");
  return nodePath;
}
...
/*
Walks the 'tree' calling back to node_found with each found node.
@param {function} node_found, callback containing a found node and a callback function that must be called once the node has been processed.
@param {function} complete - Standard node.js callback - function(err), receives an error if there was one, otherwise nothing.
*/
CeresTree.prototype.walk = function( node_found, completed ) {
  var that= this;
  walk(that.root, function(err, results){
    async.eachSeries( results, function(item, cb){
      var node= that.getNode( that.getNodePath( item ), function(node) {
        node_found( node, cb );
      });
    },
    function(err){
