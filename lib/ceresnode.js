...
CeresNode.prototype.read= function(fromTime, untilTime, cb) {
  ...
  function doRead() {
    ...
    function slicesAllRead() {
      ...
      if( earliestData ) {
        var missing= (earliestData - fromTime) / biggest_timeStep;
        var newValues= [];
      ...
      // The end of the requested interval predates all slices
         ...
         var missing= Math.floor((untilTime - fromTime) / biggest_timeStep);
         for(var i=0;i<missing;i++) resultValues.push(null);
      }

      cb( null, new TimeSeriesData(fromTime, untilTime, biggest_timeStep, resultValues));
    }
    // Normalize the timestamps to fit proper intervals
    fromTime = Math.floor(fromTime - (fromTime % biggest_timeStep) + biggest_timeStep);
    untilTime = Math.floor(untilTime - (untilTime % biggest_timeStep) + biggest_timeStep);
          ...
          if( fromTime >= nextSlice.startTime ) {
            var series = nextSlice.read( fromTime, untilTime, function(err, series) {
              if( err && err instanceof CeresErrors.NoData ) {
                ...
                if( nextSlice.timeStep < biggest_timeStep ) {
                  series.values= recalculateSeries(that.metadata.aggregationMethod, series.values, nextSlice.timeStep, biggest_timeStep);
                }

                // do something with the data
                earliestData = series.startTime

                var missing= Math.floor((untilTime - series.endTime) / biggest_timeStep);
          ...
          // or if slice contains data for part of the requested interval
            ...
            var series = nextSlice.read( nextSlice.startTime, requestUntilTime, function(err, series) {
              if( err && err instanceof CeresErrors.NoData ) {
                ...
                earliestData = series.startTime;
                var missing= Math.floor((requestUntilTime - series.endTime) / biggest_timeStep);
                var newValues = [];
                for(var k in series.values) newValues.push( series.values[k] );
                for(var i=0;i<missing;i++) newValues.push(null);
                for(var k in resultValues) newValues.push(resultValues[k]);
  ...
  function findBiggestTimeStep() {
    that.readSlices(function(err, results) {
      if( err ) cb(err);
      else {
        try {
          results.each( function(slice_tmp) {
            if( fromTime >= slice_tmp[0] ){
              if( biggest_timeStep < slice_tmp[1] ) biggest_timeStep = slice_tmp[1];
              throw sloth.StopIteration;
            }
            else if( untilTime >= slice_tmp[0] ) {
              if( biggest_timeStep < slice_tmp[1] ) biggest_timeStep = slice_tmp[1];
            }
          })
        } 
