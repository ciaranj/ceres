var  async= require("async")
   ...
   var recalculateSeries= function(aggregationmethod, values, old_timeStep, new_timeStep) {
     var factor= Math.floor(new_timeStep/old_timeStep);
     var new_values= [];
     //TOOD: This algorithm should match the aggregation logic in the rollup code
     //ideally.  (utilise XFF)
     var sub_array= [];
     var sub_array_cnt= 0;
     for(var k=0;k<values.length;k++) {
       if( values[k] == null || typeof(values[k]) == 'undefined' ) {
         // Drop the value
       }
       else {
         sub_array.push(values[k]);
       }
       sub_array_cnt++;
       if( sub_array_cnt == factor ) {
         if( sub_array.length >0 ) {
           new_values.push( utils.aggregateValues(aggregationmethod, sub_array) );
         }
         else {
           new_values.push( null );
         }
         sub_array= [];
         sub_array_cnt= 0;
       }
...
CeresNode.prototype.getTimeStepForTimeStamp= function( timeStamp, cb ) {
  var that= this;
  function getTimeStep() {
    if( that.metadata.retentions ) {
      var now = Math.floor( new Date() /1000 );
      var timeInPast= now - timeStamp;

      var retensionsClone= that.metadata.retentions.slice(0);
      retensionsClone.sort( function(a,b) {
        return a[0] - b[0];
      });
      var farBack= 0;
      var timeStep= 0;
      for(var ts in retensionsClone ) {
        farBack += (retensionsClone[ts][0] * retensionsClone[ts][1]);

        if( timeInPast <= farBack ) {
          timeStep= retensionsClone[ts][0];
          break;
        }
      }
      if( timeStep == 0 ) {
        // If this timestamp is before anything we have a retention for return the *least* precise timestep
        timeStep= retensionsClone[retensionsClone.length-1][0];
      }
      cb( null, timeStep );
    }
    else {
      cb( null, that.timeStep );
    }
  };
...
CeresNode.prototype.read= function(fromTime, untilTime, cb) {
  var that= this;
  var biggest_slice_timeStep= 1;
  var expected_retention_timeStep= 1;
  var sliceIterators;
  ...
  function doRead() {
    ...
    function slicesAllRead() {
      ...
      var ts= biggest_slice_timeStep;
      if( earliestData ) {
        var missing= (earliestData - fromTime) / biggest_slice_timeStep;

        var newValues= [];
        if( missing > 0) { newValues[missing-1]= null; }
        resultValues= newValues.concat( resultValues );
        // We need to re-sample the data down (I tried pre-calculating the correct timestep up front, but failed on the maths front,
        // this is much less efficient, but will work, allowing me to test-case it up before trying other things.)
        if( biggest_slice_timeStep != expected_retention_timeStep ) {
          resultValues= recalculateSeries(that.metadata.aggregationMethod, resultValues, biggest_slice_timeStep, expected_retention_timeStep);
          ts= expected_retention_timeStep;
        }
      }
      // The end of the requested interval predates all slices so fill with datavalues
      // of the expected timestep (expected by looking at retention archives vs the requested fromTime.)
      else {
         ts= expected_retention_timeStep;
         var missing= Math.floor((untilTime - fromTime) / ts);
         if( missing > 0 ) {
           var position= resultValues.length + missing - 1;
           resultValues[position]= null;
         }
      }

      cb( null, new TimeSeriesData(fromTime, untilTime, ts, resultValues));
    }
    // Normalize the timestamps to fit proper intervals
    fromTime = Math.floor(fromTime - (fromTime % biggest_slice_timeStep) + biggest_slice_timeStep);
    untilTime = Math.floor(untilTime - (untilTime % biggest_slice_timeStep) + biggest_slice_timeStep);
    
    var nextSlice;
    function carryOn() {
      try {
        nextSlice= sliceIterators[1].next();
      }
      catch(e) {
        slicesAllRead();
        return;
      ...
      if( fromTime >= nextSlice.startTime ) {
        var series = nextSlice.read( fromTime, untilTime, function(err, series) {
          if( err && err instanceof CeresErrors.NoData ) {
            ...
            if( nextSlice.timeStep < biggest_slice_timeStep ) {
              series.values= recalculateSeries(that.metadata.aggregationMethod, series.values, nextSlice.timeStep, biggest_slice_timeStep);
            }

            // do something with the data
            earliestData = series.startTime
            var missing= Math.floor((untilTime - series.endTime) / biggest_slice_timeStep);
            var rightNulls= missing - resultValues.length;
            if( rightNulls > 0 ) {
              series.values[series.values.length + rightNulls -1]= null;
            }
            resultValues= series.values.concat(resultValues);
      ...
      // or if slice contains data for part of the requested interval
        ...
        var series = nextSlice.read( nextSlice.startTime, requestUntilTime, function(err, series) {
          if( err && err instanceof CeresErrors.NoData ) {
            ...
            var missing= Math.floor((requestUntilTime - series.endTime) / biggest_slice_timeStep);
            if( missing > 0 ) {
              series.values[series.values.length + missing -1]= null;
            }
            resultValues= series.values.concat(resultValues);
  ...
  function findBiggestSliceTimeStep() {
    that.getTimeStepForTimeStamp(fromTime, function(err, retention_timeStep) {
      expected_retention_timeStep= retention_timeStep;
      that.slices(function(err, results) {
        if( err ) cb(err);
        else {
          try {
            // To avoid repeatedly searching the directory, we 'tee' the iterator
            // so we can re-use the non-file bit... .. this needs tidying up.. roll on generators.
            sliceIterators= results.tee(2);
            sliceIterators[0].each( function(slice_tmp) {
              if( fromTime >= slice_tmp.startTime ){
                if( biggest_slice_timeStep < slice_tmp.timeStep ) biggest_slice_timeStep = slice_tmp.timeStep;
                throw sloth.StopIteration;
              }
              else if( untilTime >= slice_tmp.startTime ) {
                if( biggest_slice_timeStep < slice_tmp.timeStep ) biggest_slice_timeStep = slice_tmp.timeStep;
              }
            })
          } 
  ...
  if(!this.fsPath) cb( new CeresErrors.NodeDeleted() )
    ...
    fs.readdir( this.fsPath, function(err, files) {
      if( err ) cb(err);
        ...
        slice_infos.sort(function(a,b) {
          return b[0]-a[0];
        });
        cb( null, sloth.ify(slice_infos) );
      }
    })
  }
}
